...
if ( ! class_exists( 'YITH_WRVP_Admin' ) ) {
	...
	class YITH_WRVP_Admin {
		 ...
		 * @since 1.0.0
		 */
		public $doc_url = 'http://yithemes.com/docs-plugins/yith-woocommerce-recently-viewed-products/';
		...
		public function action_links( $links ) {
			$links[] = '<a href="' . admin_url( "admin.php?page={$this->_panel_page}" ) . '">' . __( 'Settings', 'yith-woocommerce-recently-viewed-products' ) . '</a>';
			if( ! ( defined( 'YITH_WRVP_PREMIUM' ) && YITH_WRVP_PREMIUM ) ) {
				$links[] = '<a href="' . $this->get_premium_landing_uri() . '" target="_blank">' . __( 'Premium Version', 'yith-woocommerce-recently-viewed-products' ) . '</a>';
			}
		...
		public function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
			...
			if ( defined( 'YITH_WRVP_INIT') && YITH_WRVP_INIT == $plugin_file ) {
				$plugin_meta[] = '<a href="' . $this->doc_url . '" target="_blank">' . __( 'Plugin Documentation', 'yith-woocommerce-recently-viewed-products' ) . '</a>';
			}
