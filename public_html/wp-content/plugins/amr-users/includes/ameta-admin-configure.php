...
function amrmeta_before_after_fields( $listindex = 1) { // 201616
...
function amrmeta_listfields( $listindex = 1) {
	...
	$tabs['select']	= __('Select Fields', 'amr-users');
	$tabs['sort'] 	= __('Sort List', 'amr-users');
	$tabs['before_after'] 	= __('Before &amp; After', 'amr-users');
	
	if (isset($_GET['tab'])) {
		if ($_GET['tab'] == 'before_after'){  //nlr
			amr_users_do_tabs ($tabs,'before_after');
			amrmeta_before_after_fields ( $listindex = 1);
			return;
		}
	}
